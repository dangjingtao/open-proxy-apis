...
async function requestToken(refreshToken: string) {
  ...
  const result = await (async () => {
    ...
    const { access_token, refresh_token } = checkResult(result, refreshToken);
...
async function createConversation(
  ...
  const { id: convId } = await request("POST", "/api/chat", refreshToken, {
    data: {
...
async function createCompletion(
  ...
  return (async () => {
    ...
    // 创建会话
    const convId = /[0-9a-zA-Z]{20}/.test(refConvId)
      ? refConvId
      : await createConversation(model, "未命名会话", refreshToken);

    // 提取引用文件URL并上传kimi获得引用的文件ID列表
    const refFileUrls = extractRefFileUrls(messages);
    const refResults = refFileUrls.length
      ? await Promise.all(
          refFileUrls.map((fileUrl) =>
            uploadFile(fileUrl, refreshToken, convId)
          )
        )
      : [];
    ...
    // 伪装调用获取用户信息
    fakeRequest(refreshToken).catch((err) => logger.error(err));

    // 消息预处理
    const sendMessages = messagesPrepare(messages, !!refConvId);

    !segmentId &&
      preN2s(model, sendMessages, refs, refreshToken, convId).catch((err) =>
        logger.error(err)
      );
    getSuggestion(sendMessages[0].content, refreshToken).catch((err) =>
      logger.error(err)
    );
    tokenSize(sendMessages[0].content, refs, refreshToken, convId).catch(
    ...
    logger.info(
      `使用模型: ${model}，是否联网检索: ${isSearchModel}，是否探索版: ${isResearchModel}，是否K1模型: ${isK1Model}，是否数学模型: ${isMath}`
    );
    ...
    if (isResearchModel) {
      ...
      if (used >= total)
        throw new APIException(
          EX.API_RESEARCH_EXCEEDS_LIMIT,
          `探索版使用量已达到上限`
        );
      logger.info(`探索版当前额度: ${used}/${total}`);
    }

    const kimiplusId = isK1Model
      ? "crm40ee9e5jvhsn7ptcg"
      : /^[0-9a-z]{20}$/.test(model)
      ? model
      : "kimi";
    ...
    const stream = await request(
      "POST",
      `/api/chat/${convId}/completion/stream`,
      refreshToken,
      {
        data: segmentId
          ? {
    ...
    if (answer.choices[0].finish_reason == "length" && answer.segment_id) {
      const continueAnswer = await createCompletion(
        model,
        [],
        refreshToken,
        convId,
      ...
      answer.choices[0].message.content +=
        continueAnswer.choices[0].message.content;
    }
...
function messagesPrepare(messages: any[], isRefConv = false) {
  ...
  if (isRefConv || messages.length < 2) {
    content = messages.reduce((content, message) => {
      ...
      return (content += `${
        message.role == "user"
          ? wrapUrlsToTags(message.content)
          : message.content
      }\n`);
    }, "");
    logger.info("\n透传内容：\n" + content);
...
function checkResult(result: AxiosResponse, refreshToken: string) {
  ...
  if (error_type == "auth.token.invalid") accessTokenMap.delete(refreshToken);
