...
async function createCompletion(
  ...
  return (async () => {
    ...
    // 如果引用对话ID不正确则重置引用
    if (!/[0-9a-z]{32}/.test(refConvId)) refConvId = "";
...
async function uploadFile(fileUrl: string, ticket: string) {
  ...
  if (isImage) {
    ...
    if (!data.results[0] || !data.results[0].url)
      throw new Error(
        `文件上传失败：${
          data.results[0] ? data.results[0].errorMsg : "未知错误"
        }`
      );
    ...
    while (true) {
      ...
      if (data.pollEndFlag) {
        if (data.statusList[0] && data.statusList[0].status === 0)
          throw new Error(
            `文件处理失败：${data.statusList[0].errorMsg || "未知错误"}`
          );
        break;
      }
...
function generateCookie(ticket: string) {
  return [
    `${
      ticket.length > 100 ? "login_aliyunid_ticket" : "tongyi_sso_ticket"
    }=${ticket}`,
    "aliyun_choice=intl",
