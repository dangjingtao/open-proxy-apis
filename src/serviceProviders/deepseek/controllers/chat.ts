...
function generateCookie() {
  return `intercom-HWWAFSESTIME=${util.timestamp()}; HWWAFSESID=${util.generateRandomString(
    ...
    false
  )}=${util.unixTimestamp()},${util.unixTimestamp()},${util.unixTimestamp()}; Hm_lpvt_${util.uuid(
    false
  )}=${util.unixTimestamp()}; _frid=${util.uuid(false)}; _fr_ssid=${util.uuid(
    false
  )}; _fr_pvid=${util.uuid(false)}`;
}
...
async function answerChallenge(
  ...
  const { algorithm, challenge, salt, difficulty, expire_at, signature } =
    response;
...
async function createCompletion(
  ...
  return (async () => {
    ...
    // 如果引用对话ID不正确则重置引用
    if (!/[0-9a-z\-]{36}@[0-9]+/.test(refConvId)) refConvId = null;
    ...
    // 创建会话
    const sessionId =
      refSessionId || (await createSession(model, refreshToken));
    // 请求流
    const token = await acquireToken(refreshToken);

    const isSearchModel =
      model.includes("search") || prompt.includes("联网搜索");
    const isThinkingModel =
      model.includes("think") ||
      model.includes("r1") ||
      prompt.includes("深度思考");
    ...
    const challengeResponse = await getChallengeResponse(
      refreshToken,
      "/api/v0/chat/completion"
    );
    const challenge = await answerChallenge(
      challengeResponse,
      "/api/v0/chat/completion"
    );
...
async function receiveStream(
  ...
  logger.info(
    `模型: ${model}, 是否思考: ${isThinkingModel} 是否联网搜索: ${isSearchModel}, 是否静默思考: ${isSilentModel}, 是否折叠思考: ${isFoldModel}`
  );
  let refContent = "";
    ...
    const parser = createParser((event) => {
      try {
        ...
        if (!result.choices || !result.choices[0] || !result.choices[0].delta)
          return;
        if (!data.id) data.id = `${refConvId}@${result.message_id}`;
        if (
          result.choices[0].delta.type === "search_result" &&
          !isSilentModel
        ) {
        ...
        if (result.choices[0].delta.type === "thinking") {
          if (!thinking && isThinkingModel && !isSilentModel) {
            ...
            data.choices[0].message.content += isFoldModel
              ? "<details><summary>思考过程</summary><pre>"
              : "[思考开始]\n";
          }
    ...
    try {
      ...
      if (result.choices[0].delta.type === "search_result" && !isSilentModel) {
        ...
        if (searchResults.length > 0) {
          const refContent =
            searchResults
              .map((item) => `检索 ${item.title} - ${item.url}`)
              .join("\n") + "\n\n";
          transStream.write(
            `data: ${JSON.stringify({
      ...
      if (!result.choices[0].delta.content) return;
          ...
          choices: [
            {
              ...
              delta: {
                role: "assistant",
                content: result.choices[0].delta.content.replace(
                  /\[citation:\d+\]/g,
                  ""
                ),
              },
      ...
      "https://chat.deepseek.com/api/v0/events",
      {
        events: [
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 100 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 200 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 300 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 400 + Math.floor(Math.random() * 1000),
            event_name: "__httpResponse",
            event_message: `httpResponse POST /api/v0/chat_session/create, ${Math.floor(
              Math.random() * 1000
            )}ms, reason: none`,
            payload: {
              __location: "https://chat.deepseek.com/",
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 600 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 700 + Math.floor(Math.random() * 1000),
            event_name: "__httpRequest",
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 800 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 900 + Math.floor(Math.random() * 1000),
            event_name: "createSessionAndStartCompletion",
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 1000 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 1100 + Math.floor(Math.random() * 1000),
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 1200 + Math.floor(Math.random() * 1000),
            event_name: "__tti",
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 1300 + Math.floor(Math.random() * 1000),
            event_name: "__httpResponse",
          ...
          },
          {
            session_id: sessionId,
            client_timestamp_ms:
              timestamp + 1400 + Math.floor(Math.floor(Math.random() * 1000)),
